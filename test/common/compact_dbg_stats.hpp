...
void print_graph_nodes(
	...
	if (all_has_data > 0) {
		...
		BL_BENCH_COLLECTIVE_END(node_print, "psort nodes", branch_pts.size(), comm);   // this is for ordered output.
	}
	...
	BL_BENCH_COLLECTIVE_END(node_print, "print graph nodes (debug)", branch_pts.size(), comm);

	BL_BENCH_REPORT_MPI_NAMED(node_print, "graph_print", comm);

}

void print_graph_edges(
		std::string const & filename,
		std::vector<::bliss::debruijn::biedge::compact_simple_biedge_kmer_node<KmerType>> & branch_pts,
		mxx::comm const & comm) {
	...
	if (all_has_data > 0) {
		...
		if (has_data == 1) {
			mxx::sort(branch_pts.begin(), branch_pts.end(),
				[](::bliss::debruijn::biedge::compact_simple_biedge_kmer_node<KmerType> const & x,
					::bliss::debruijn::biedge::compact_simple_biedge_kmer_node<KmerType> const & y){
				return x.first < y.first;
			}, subcomm);
		}
		BL_BENCH_COLLECTIVE_END(node_print, "psort nodes", branch_pts.size(), comm);   // this is for ordered output.
...
void print_graph_edge_frequencies(
	...
	// sort the branches
	BL_BENCH_START(branch_print);
	print_graph_nodes(filename, branch_pts, comm);
